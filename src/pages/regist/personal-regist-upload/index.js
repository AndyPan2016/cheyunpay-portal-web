...
export default {
  data () {
    return {
      uploadLists: [
        { text: '身份证正面', key: 'certFrontPath', background: require('../../../assets/images/background/bg-idcard-upload-a.png') },
        { text: '身份证反面', key: 'certBackPath', background: require('../../../assets/images/background/bg-idcard-upload-b.png') }
      ],
      uploadButtons: [
  ...
  methods: {
    ...
    uploadSuccess (file, rows, fileItem, dataIdx) {},
    registSubmit () {
      let uploadLists = this.uploadLists
      let file
      let key
      let falseItem
      let params = this.transfToJson(this.getLocalStorage(STATUS.PERSONALREGIST)) || {}
      ...
      if (falseItem) {
        ...
        RegistService.personRegister(params).then(res => {
          let data = res.data
          if (data.success) {
